...
import math
from ChannelAug import ChannelAdap, ChannelAdapGray, ChannelRandomErasing,ChannelExchange,Gray,RGB2HSV,IR2HSV
...
def get_train_loader_ir(args, dataset, height, width, batch_size, workers,
    ...
    train_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=dataset.images_dir, transform=train_transformer),
                   ...
                   shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)#####img, fname, pid, camid, index
    ...
    if train_transformer1 is None:
        ...
        train_loader = IterLoader(
            DataLoader(Preprocessor_color(train_set, root=dataset.images_dir, transform=train_transformer,transform1=train_transformer1),
                       batch_size=batch_size, num_workers=workers, sampler=sampler,
                       shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)######img, img1,fname, pid, camid, index
...
def main_worker_stage1(args,log_s1_name):
    ...
    for epoch in range(args.epochs):
        ...
        transform_RGB2HSV = T.Compose([
            T.Resize((height, width), interpolation=3),
            T.Pad(10),
            T.RandomCrop((height, width)),
            T.RandomHorizontalFlip(p=0.5),
            RGB2HSV(probability=0.5),
            T.ToTensor(),
            normalizer,
            # ChannelRandomErasing(probability=0.5),
            # ChannelExchange(gray=2)
        ])
        transform_IR2HSV = T.Compose([
            T.Resize((height, width), interpolation=3),
            T.Pad(10),
            T.RandomCrop((height, width)),
            T.RandomHorizontalFlip(p=0.5),
...
##############################
            ...
            mode='all'
            data_path='/opt/data/private/lg/USL-VI-ReID/RegDB/'
...
def main_worker_stage2(args,log_s1_name,log_s2_name):
    ...
    for epoch in range(args.epochs):
        ...
        train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                        args.batch_size, args.workers, args.num_instances, iters,
                                        trainset=pseudo_labeled_dataset_ir, no_cam=args.no_cam,train_transformer=transform_IR2HSV)
