...
import math
from ChannelAug import ChannelAdap, ChannelAdapGray, ChannelRandomErasing, ChannelExchange, Gray,IR2HSV,IR2HSV2
...
def main_worker_stage1(args, log_s1_name):
    ...
    train_transformer_rgb = T.Compose([
        ...
        T.RandomApply([T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)], p=0.6),
        T.ToTensor(),
        normalizer,
    ...
    for epoch in range(args.epochs):
        ...
        if epoch >= 6 and ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            mode = 'all'
            data_path = '/opt/data/private/lg/USL-VI-ReID/SYSU-MM01'
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    for epoch in range(args.epochs):
        ...
        #         s = unmatched_cost.shape[0] - unmatched_cost.shape[1]
        #         if s > 0:
        ...
        #             else:
        #                 unmatched_row2.append(unmatched_row[idx])
        #     if s > 0:
        #         unmatched_cost2 = cost[unmatched_row2][:, :dis_similarity.shape[1]]
        #         unmatched_row_ind2, unmatched_col_ind2 = linear_sum_assignment(unmatched_cost2)
        #         for idx, item in enumerate(unmatched_row_ind2):
        #             R.append((unmatched_row2[idx], unmatched_col_ind2[idx]))
        #             r2i[unmatched_row2[idx]] = unmatched_col_ind2[idx]
        ...
        train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                              args.batch_size, args.workers, args.num_instances, iters,
                                              trainset=pseudo_labeled_dataset_ir, no_cam=args.no_cam,
                                              train_transformer=transform_thermal1)
